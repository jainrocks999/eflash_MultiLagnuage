...
const SettingScreen = props => {
  ...
  console.log('thisis si table', table);
  ...
  const [language, setLanguage] = useState('tbl_items');
  const [prevlanguage, setPrevLanguage] = useState('tbl_items');
  ...
  const getTable = async () => {
    const prevtab = await AsyncStorage.getItem('table');
    if (prevtab != null) {
      setPrevLanguage(prevtab);
    }
  };
        ...
        <ScrollView>
          <View
            ...
            <ImageBackground
              ...
              <View
                style={{marginTop: tablet ? '7%' : '8%', marginLeft: '-5%'}}>
                <Switch
                  text="Language"
                  style={[styles.sw, {fontFamily: 'KaiseiTokumin-ExtraBold'}]}
                  type={true}
                  onPress={async item => {
                    setLanguage(item);
          ...
          <View
            ...
            <TouchableOpacity
              onPress={async () => {
                await AsyncStorage.setItem('table', language);
                dispatch(Table(language));
                if (language != table) {
                  Navigation.reset({index: 0, routes: [{name: 'home'}]});
                } else {
                  Save();
